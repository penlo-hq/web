...
function RoleBadge({ role }: { role: string }) {
  return (
    <span className="inline-flex items-center px-2.5 py-1 rounded-full border border-text-secondary/10 bg-surface text-[10.5px] uppercase tracking-[0.16em] text-text-secondary">
      {roleLabel(role)}
    </span>
  )
}
...
export function TeamPermissions({ onMenuClick }: PageProps) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col h-screen bg-canvas">
      <TopBar onMenuClick={onMenuClick} title="Permissions" subtitle="Roles & invites" />
        ...
        <div className="max-w-2xl space-y-6">
          ...
          <section className="space-y-3">
            <div className="flex items-center justify-between">
              <h2 className="font-display font-bold text-[16px] tracking-tightest text-text-primary">Members</h2>
              {isAdmin && (
                <button
                  type="button"
                  onClick={() => setInviteOpen(true)}
                  className="px-4 py-2 rounded-xl bg-accent text-white text-[11px] uppercase tracking-[0.16em] hover:opacity-90 transition-opacity"
                >
                  Invite member
                </button>
              )}
              ...
              <ul className="space-y-2">
                {sortedMembers.map((member) => {
                  ...
                  return (
                    <li
                      key={member.id}
                      className="flex items-center justify-between px-5 py-4 rounded-xl border border-text-secondary/10 bg-white"
                    >
                      <div className="min-w-0">
                        <p className="text-[13.5px] font-medium text-text-primary truncate">
                          {member.name}
                          {isSelf && <span className="ml-2 text-[10.5px] text-text-secondary">(you)</span>}
                        </p>
                        <p className="text-[11.5px] text-text-secondary truncate">{member.email}</p>
                      </div>
                      {isAdmin && !isSelf ? (
                        <select
                          value={member.role}
                          ...
                          className="px-3 py-2 rounded-xl border border-text-secondary/10 text-[12.5px] bg-white focus:outline-none focus:border-ink disabled:opacity-50"
                        >
                          {ROLE_OPTIONS.map((r) => (
          ...
          <section className="space-y-3">
            <h2 className="font-display font-bold text-[16px] tracking-tightest text-text-primary">Pending invitations</h2>

            {!loading && invitations.length === 0 && (
              <div className="px-6 py-8 rounded-xl border border-text-secondary/10 bg-surface text-center">
                <p className="text-[13px] text-text-secondary">No pending invitations</p>
              </div>
            )}
            ...
            {!loading && invitations.length > 0 && (
              <ul className="space-y-2">
                {invitations.map((invite) => (
                  ...
                  >
                    <div className="min-w-0">
                      <p className="text-[13.5px] font-medium text-text-primary truncate">{invite.email}</p>
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                      ...
                      {isAdmin && (
                        <button
                          ...
                          className="text-[10.5px] uppercase tracking-[0.16em] text-text-secondary hover:text-text-primary transition-colors disabled:opacity-50"
                        >
